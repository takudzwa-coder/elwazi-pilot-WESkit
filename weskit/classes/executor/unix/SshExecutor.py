...
class SshExecutor(Executor):
    ...
    async def _wait_for(self, process: ExecutedProcess) -> CommandResult:
        if process.id.value is None:
            ...
            try:
                ...
                raise WorkLoadError(f"Process {process.id.value} timed out:" +
    ...
    def kill(self, process: ExecutedProcess):
        try:
            ...
            raise WorkLoadError(f"Could not kill process ({process.command.command})", e)

...
class AbstractStateMapper(Generic[S], metaclass=ABCMeta):
    ...
    `ExecutionState` also from `ForeignState.reasons` or `ForeignState.exit_code`.
    ...
    def _accept_foreign_state(
        self, execution_state: ObservedExecutionState[S], foreign_state: ForeignState[S]
    ) -> ObservedExecutionState[S]:
        """
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        Given an `ExecutionState` and an `ForeignState`, return the possibly
...
class SimpleStateMapper(Generic[S], AbstractStateMapper[S]):
    ...
    if the next state WESkit ExecutionState depends only on the `ForeignState`, but not, e.g.
    ...
    def __init__(self, state_map: Dict[S | None, Type[ObservedExecutionState[S]]]):
        """
        The state_map should map `ForeignState` instances to `ExecutionState` classes. The

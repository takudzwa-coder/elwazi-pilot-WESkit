...
class Local:
        ...
        if os.path.getmtime(self.credentialsFile) > self.lastupdate:
            ...
            with open(self.credentialsFile, 'r') as stream:
                try:
                    ...
                    self.dict={key:User(key,value,self.authtype) for key,value in ystream.items() if type(value) is dict and  len({'roles','password','salt'}.intersection(value))==3}
                    
                    if(len(self.dict)) !=len(ystream):
                        print("Warning: Skiped %d Entrys from DBfile: Invalid Format!"%(len(ystream)-len(self.dict)))
    ...
    def authenticate(self,username,password):
        ...
        self.updateCredentials()
        if not username in self.dict:
            return(None)
        user=self.dict[username]

        if hashlib.sha256((password+user.salt).encode('utf-8')).hexdigest()==user.password:
            return(user)

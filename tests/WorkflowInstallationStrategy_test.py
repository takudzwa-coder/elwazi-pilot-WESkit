...
def test_parse_trs_uri():
    info = WorkflowInfo.from_uri_string("trs://server/workflow/version/SMK")
    assert info.name == "workflow"
    assert info.version == "version"
    assert info.type == "SMK"
    assert info.workflow_type == "SMK"
    assert info.primary_file is None

    info = WorkflowInfo.from_uri_string("trs://server:1234/workflow/version/PLAIN_SMK")
    assert info.type == "PLAIN_SMK"
    assert info.workflow_type == "SMK"

    info = WorkflowInfo.from_uri_string("trs://server/workflow/version/NFL")
    assert info.type == "NFL"
    assert info.workflow_type == "NFL"

    info = WorkflowInfo.from_uri_string("trs://server/workflow/version/PLAIN_NFL")
